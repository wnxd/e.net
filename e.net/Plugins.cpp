...
MethodDefinition^ MethodClone(ModuleDefinition^ module, ModuleDefinition^ M, MethodDefinition^ method)
{
	...
	if (method->HasBody)
	{
		...
		for each (Instruction^ ins in method->Body->Instructions)
		{
			...
			else if (ins->OpCode == OpCodes::Castclass || ins->OpCode == OpCodes::Box || ins->OpCode == OpCodes::Unbox || ins->OpCode == OpCodes::Unbox_Any || ins->OpCode == OpCodes::Newarr || ins->OpCode == OpCodes::Ldelema || ins->OpCode == OpCodes::Initobj) ins->Operand = GetTypeReference(module, M, dynamic_cast<TypeReference^>(ins->Operand));
